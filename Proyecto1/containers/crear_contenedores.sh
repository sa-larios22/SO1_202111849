...
for i in {1..10}; do
    ...
    case $tipo in
        "RAM")
            docker run -d --rm --name $nombre containerstack/alpine-stress stress --vm 1 --vm-bytes 128M --timeout 180s
            ;;
        "CPU")
            docker run -d --rm --name $nombre containerstack/alpine-stress stress --cpu 1 --timeout 180s
            ;;
        "IO")
            docker run -d --rm --name $nombre containerstack/alpine-stress stress --io 1 --timeout 180s
            ;;
        "DISK")
            docker run -d --rm --name $nombre containerstack/alpine-stress stress --hdd 1 --hdd-bytes 256M --timeout 180s
            ;;
    esac
